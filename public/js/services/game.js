angular.module('mean.system')
  .factory('game', ['socket', '$http', '$timeout', function (socket, $http, $timeout) {
    ...
    // That way, we don't trigger the $scope.$watch too often
      ...
      for (i = 0; i < data.players.length; i++) {/* eslint-disable-line */
      ...
      if (data.table.length === 0) {
        ...
        for (i = 0; i < added.length; i++) {/* eslint-disable-line */
          for (let j = 0; j < data.table.length; j++) {/* eslint-disable-line */
            if (added[i] === data.table[j].player) {
              game.table.push(data.table[j], 1);
            }
          }
        }
        for (i = 0; i < removed.length; i++) {/* eslint-disable-line */
          for (let k = 0; k < game.table.length; k++) {/* eslint-disable-line */
